...
#define TYPE_HSPTEST_SIC "hsptest_sic"
OBJECT_DECLARE_SIMPLE_TYPE(hsptest_sic_state, HSPTEST_SIC)

struct hsptest_sic_state {
...
static const VMStateDescription vmstate_hsptest_sic = {
    .name = "hsptest_sic",
    .version_id = 1,
    .minimum_version_id = 1,
    .fields = (VMStateField[]) {
        VMSTATE_UINT32(level, hsptest_sic_state),
        VMSTATE_UINT32(mask, hsptest_sic_state),
        VMSTATE_UINT32(pic_enable, hsptest_sic_state),
        VMSTATE_END_OF_LIST()
    }
};

static void vpb_sic_update(hsptest_sic_state *s)
{
    uint32_t flags;
...
static void vpb_sic_update_pic(hsptest_sic_state *s)
{
    int i;
    uint32_t mask;
...
{
    hsptest_sic_state *s = (hsptest_sic_state *)opaque;
    if (level)
        s->level |= 1u << irq;
    else
        s->level &= ~(1u << irq);
...
static void hsptest_sic_init(Object *obj)
{
    DeviceState *dev = DEVICE(obj);
    hsptest_sic_state *s = HSPTEST_SIC(obj);
...
static void versatile_init(MachineState *machine, int board_id)
{
    ...
    dev = sysbus_create_simple(TYPE_HSPTEST_SIC, 0x10003000, NULL);
    for (n = 0; n < 32; n++) {
...
static void hsptest_init(MachineState *machine)
{
    versatile_init(machine, 0x183);
}

static void hsptest_class_init(ObjectClass *oc, void *data)
{
    MachineClass *mc = MACHINE_CLASS(oc);

    mc->desc = "ARM HSP Test";
    mc->init = hsptest_init;
...
static const TypeInfo hsptest_type = {
    .name = MACHINE_TYPE_NAME("hsptest"),
    .parent = TYPE_MACHINE,
    .class_init = hsptest_class_init,
};

static void hsptest_machine_init(void)
{
    type_register_static(&hsptest_type);
}

type_init(hsptest_machine_init)

static void hsptest_sic_class_init(ObjectClass *klass, void *data)
{
    DeviceClass *dc = DEVICE_CLASS(klass);

    dc->vmsd = &vmstate_hsptest_sic;
}

static const TypeInfo hsptest_sic_info = {
    .name          = TYPE_HSPTEST_SIC,
    .parent        = TYPE_SYS_BUS_DEVICE,
    .instance_size = sizeof(hsptest_sic_state),
    .instance_init = hsptest_sic_init,
    .class_init    = hsptest_sic_class_init,
};

static void hsptest_register_types(void)
{
    type_register_static(&hsptest_sic_info);
}

type_init(hsptest_register_types)
